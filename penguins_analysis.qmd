...
p <- ggplot(data = penguins_data_from_csv, aes(x = species, y = body_mass_g, fill = species)) +
  geom_boxplot() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_viridis_d()
...
```{r}
ggplot(data = average_species_culmen_depth, aes(x = species, y = average_culmen_depth, fill = species)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  labs(x = "Species", y = "Average Culmen Depth (mm)", title = "Average Culmen Depth by Penguin Species") +
  scale_fill_viridis_d()
```
