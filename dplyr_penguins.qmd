...
penguins %>%
  ...
  group_by(island) %>%
  summarise(flip_max_length = max(flipper_length_mm),
...
penguins %>%
  group_by(island) %>%
  summarise(across(ends_with("mm"), median, na.rm = TRUE, .names = "{.col}_median"))
```
...
penguins %>%
  ...
  summarise(across(contains("length"), mean, na.rm = TRUE, .names = "{.col}_max")) 
```
